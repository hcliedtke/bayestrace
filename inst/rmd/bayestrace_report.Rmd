...
log_path<-list.files(path=file_path, pattern=".Log.txt", full.names = T)
names(log_path)<-basename(log_path)

sched_path<-list.files(path=file_path, pattern=".Schedule.txt", full.names = T)
names(sched_path)<-basename(sched_path)

stones_path<-list.files(path=file_path, pattern=".Stones.txt", full.names = T)
names(stones_path)<-basename(stones_path)

tree_path<-paste0(file_path,"/",
                      pattern=read_bt_header(log_path[1]) %>%
                        filter(Options=="Tree File Name") %>%
                        pull(2))

traits_path <- paste0(file_path,"/",
                      pattern=read_bt_header(log_path[1]) %>%
                        filter(Options=="Data File Name") %>%
                        pull(2))
