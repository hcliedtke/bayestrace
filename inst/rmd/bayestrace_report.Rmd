...
abbrev_length<-params$abbrev_length
...
# add more colours
more_colors<-colours()[seq(3, 201, by=7)]
names(more_colors)<-as.character(7:35)
...
bt<-read_bt_log(file_path = log_path,abbrev_length = abbrev_length)
...
# load schedule files
acceptance_rate<-read_bt_schedule(file_path = sched_path, abbrev_length = abbrev_length)

# load stones files
if(length(stones_path)>0){
  stones<-read_bt_stones(file_path = stones_path, abbrev_length = abbrev_length)
}
```
