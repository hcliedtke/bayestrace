...
### TEST DATASET
#file_path="~/Desktop/bayestrace_test/Artiodactyl_multistates_anc_states/"
#chains_burnin=0
#downsample=5000
...
tr_name<-read_bt_log(log_path[1], read_chains = F)$header %>%
...
comparison_mode<-bt$header %>%
  filter(rowSums(across(2:ncol(.), ~ .x != bt$header[, 2])) > 0) %>%
...
bt$header %>%
  mutate(not_same = rowSums(across(2:ncol(.), ~ .x != bt$header[[2]])) > 0) %>% # identify columns that are not the same
  reactable(
    defaultPageSize = 50,
...
```{r mc-check}
# check if stones file exists and is empty
st<-exists("stones")
