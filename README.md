...
If the same mode was run repeatedly, to check convergence of the MCMC chain, or with slightly different parameters to check e.g. prior sensitivity, these multiple runs (of the same mode) can be uploaded into a single BayesTrace report.
...
```{}
devtools::install_github("hcliedtke/bayestrace")
library(bayestrace)
```

To load a BayesTraits log file

```{}
BT_logout<-read_bt_log("path/to/Log.txt")
```

To generate the BayesTrace report
