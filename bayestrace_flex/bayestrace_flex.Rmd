...
#setwd("~/Documents/bayestrace/bayestrace_flex/")
...
# load input tree
tre<-read.nexus(file = paste0(dir_path,"/",bt$header %>%
...
## add trait data to first tree
tre_1<-tre[[1]]
tre_1$traits<-dat %>%
  arrange(match(Species, tre_1$tip.label)) %>%
  pull(Trait)
}
```
...
### Reverse Jump Model Sampling {data-height=20%}
...
### Reverse Jump Model Ranking {data-height=20%}
...
### Acyclic diagrams of three most frequently sampled model
