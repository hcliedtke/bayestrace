...
log_path<-params$file_index %>%
  filter(filetype=="log") %>%
  select(filename, filepath) %>%
  deframe()

sched_path<-params$file_index %>%
  filter(filetype=="schedule") %>%
  select(filename, filepath) %>%
  deframe()

stones_path<-params$file_index %>%
...
# load schedule files
acceptance_rate<-read_bt_schedule(sched_path)

# load stones files
if(any(file_index$filetype %in% "stones")){
  stones<-read_bt_stones(stones_path)
}
```
