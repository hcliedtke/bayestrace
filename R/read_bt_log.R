...
##
  read_bt_log<-function(log_path, read_chains=T){

  ## stop function if wrong files loaded

  if(!all(str_detect(log_path, "\\.Log\\.txt$"))){
  ...
  suppressMessages(suppressWarnings({
    ...
    ### read log header
    log_names<-str_remove_all(basename(log_path), pattern="\\.Log\\.txt")
    ...
    for(i in 1:length(log_path)){

      ## read the whole file ONCE per path, reuse for every section below
      all_lines<-read_lines(log_path[i], n_max=Inf)
